...
+ ~+xwidget~ Enables message view using xwidgets to render html emails.
+ ~+async~ Send messages asynchronously.
+ ~+alert~ Enable OS alerts when new email is delivered.
+ ~+modeline~ Enable email notification on emacs mode line.
...
#+BEGIN_SRC sh
mu init --maildir ~/.mail --my-address email1@example.com --my-address email2@example.com
mu index
#+END_SRC
